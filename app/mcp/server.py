...
Every other tool in this app (app/agent/tools.py) reads `SecurityCtx` out of
`RunnableConfig["configurable"]["ctx"]` — a channel that only exists because
this app's own LangGraph runtime puts it there (app/agent/runtime.py's `_config`),
itself sourced from a trusted HTTP header (app/api/main.py's `get_ctx`) that a
...
gate app/agent/tools.py's tools use, then passed straight through to
...
for the same reason app/api/main.py's docstring gives for its own trusted-header
