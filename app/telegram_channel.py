...
first-party interface alongside the CLI (app/chat.py), the HTTP API
(app/api.py), and the built-in web UI (pattern 29). Long-polls Telegram's
...
`app/agent.py::answer()` every other synchronous interface already calls —
...
Run with: `python -m app.telegram_channel` (see Makefile's `telegram`
...
def _ctx_for_user(user_id: int) -> SecurityCtx:
    ...
    the same shape as app/chat.py's local dev ctx, just keyed by Telegram's
    own user id instead of the OS username, so different Telegram users
    never share memories (app/tools.py's remember/recall_memories)."""
...
async def run() -> None:
    ...
    if not TELEGRAM_BOT_TOKEN:
        raise RuntimeError(
            "TELEGRAM_BOT_TOKEN is not set — see app/telegram_channel.py's module docstring"
        )
