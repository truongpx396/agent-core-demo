...
`app/api.py` reads tenant/principal from trusted headers as the seam a real
...
class SecurityCtx(TypedDict):
    """Stamped ONCE, at the trusted boundary (app/api.py's header extraction,
    or a local dev entry point like app/chat.py) — never by a graph node,
    ...
    app/api.py::_extract_ctx for where the line actually gets drawn.
    """
...
class Policy(Protocol):
    ...
    def lower(self, ctx: SecurityCtx, target: str) -> Filter:
        ...
        ("documents" | "memories" — the two kinds sharing app/tools.py's
        Qdrant collection). MUST be applied INSIDE the store as a
        pre-filter (see app/qdrant_store.py) — post-filtering in Python is
