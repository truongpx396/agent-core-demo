...
def _retrieval_query(current_text: str, previous_human: HumanMessage | None) -> str:
    ...
    """
    # Deferred for the same reason as _assemble_shared_graph_parts's own
    # deferred imports below: app/agent/graph_utils.py imports THIS module
    # back at its own top level, so importing it back here at graph.py's
    # own module level would close a real cycle.
...
def make_agent_node(llm):
    ...
    """
    # Deferred for the same reason as _assemble_shared_graph_parts's own
    # deferred imports below: app/agent/graph_tools.py and
    # app/agent/graph_skills.py both import State back from THIS module at
    # THEIR OWN top level, so importing either back here at graph.py's own
    # module level would close a real cycle. Only ever needed once, here,
    # at make_agent_node's own call time (inside _assemble_shared_graph_parts),
    # long after every module has finished loading — not on every turn.
...
def make_no_answer_fallback_node(emit_message: bool = True, system_prompt: str = SYSTEM_PROMPT):
    def no_answer_fallback(state: State) -> dict:
        if not emit_message:
            return {}
        # Deferred for the same reason _assemble_shared_graph_parts's own
        # graph_routing import is: app/agent/graph_routing.py imports
        # State/constants/a few helpers back from THIS module at its own
        # top level, so importing it back here at graph.py's own module
        # level would close a real cycle. Only needed here, at NODE
        # EXECUTION time, long after both modules have finished loading.
