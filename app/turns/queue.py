...
- `requests_stream_key(domain)` (`agent:requests:{domain}`) — ONE shared
  ...
  share — is what lets app/api/main.py stay ONE unified process serving
  every domain (routing each request by its `X-Domain` header, see that
  module's `get_domain`) while still keeping each domain's worker pool
  independently scalable, exactly like pattern 43's original SSE-tier/
  worker-tier split, just applied one level deeper.
- A per-request results stream (`results_stream_key(request_id)`) — created
  ...
  forever — the same TTL-on-write shape app/retrieval/semantic_cache.py already uses
...
def get_client() -> redis.Redis:
    """A separate client/connection from app/retrieval/semantic_cache.py's (module-
...
async def publish_request(
    ...
    *,
    ...
    `domain`'s own requests stream (`requests_stream_key`) — only a worker
    pool booted with a matching `AGENT_DOMAIN` ever reads it.
    `request_id` is generated by the CALLER (app/api/main.py), not here — the
    ...
    `domain` itself never rides IN the payload — which stream a message
    landed on already tells the worker that picks it up which domain it's
    for, exactly the way app/channels/telegram.py's whole process already
    commits to one domain via `AGENT_DOMAIN` rather than a per-message field.
    ...
    was built from that domain's own manifest/tools, so a different
    domain's worker resuming it would run the wrong tool universe against
    it; app/api/main.py's `X-Domain` header is what keeps all three calls
...
async def publish_resume_request(
    ...
    *,
    ...
    worker-process memory — see app/agent/runtime.py's module docstring;
    see `publish_request`'s own docstring for why `domain` must match the
    original turn's)."""
    payload = json.dumps(
        {
            "kind": "resume",
...
async def publish_cancel_request(
    ...
    (app/api/main.py) always does both: set the flag AND publish this, so
    whichever mechanism actually applies handles it. `domain` — see
    `publish_request`'s own docstring."""
    payload = json.dumps(
...
async def read_results(client: redis.Redis, request_id: str, *, block_ms: int = 5000):
    ...
    yields for a paused turn (app/agent/runtime.py) — the worker's own forwarding
