...
2. **This deployment's own bound secret values** — the actual configured
   ...
   `app.config` at call time, so an exact echo of THIS deployment's real
...
def _bound_secret_values() -> list[str]:
    ...
    fresh from app.config on every call (not cached at import time) so a
...
def scrub(text: str) -> str:
    ...
    app/moderation.py::screen already takes.
    """
    if not text:
        return text
    try:
