...
- ~~Fault-tolerant redelivery for the Redis Streams queue~~ — **done**: `queue.py::reclaim_stale_entries` (`XAUTOCLAIM`), run periodically by both `agent_worker.py` and `ingest_worker.py`'s own `_reclaim_loop`, finds entries abandoned by a worker that died mid-job. Deliberately does NOT redeliver them to run again — an in-flight job may already have executed a side-effecting tool call or upserted chunks, so re-running risks duplicating that — instead it surfaces a `WORKER_LOST`/error event on the job's own results stream and archives the payload to a dead-letter stream (`queue.py::dead_letter_stream_key`) for manual inspection/replay, then acks it. Any nonzero `agent_worker_job_reclaimed_total` rate means workers are crashing, not that recovery is silently fine — still worth alerting on.
