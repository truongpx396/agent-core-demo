...
1. `agent()`'s assembled message list is byte-identical for two different
   ...
   the message list through app/graph.py's own code.
...
from app.graph import SYSTEM_PROMPT, make_agent_node
...
class _RecordingFakeLLM:
    ...
    invoked with — see tests/test_agent_node.py for why this is a plain
...
class TestNoCtxLeakIntoMessageContent:
    ...
    def test_claims_dict_never_leaks_into_message_content(self):
        """`claims` is the opaque, domain-specific bag (app/security.py) —
        nothing in app/graph.py should ever stringify it into a prompt."""
