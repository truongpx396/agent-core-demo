...
app/agent/runtime.py's matching comments on both functions.
...
already covered by tests/agent/test_durable_checkpoint.py) so these stay fast
...
def _install_fake_graph(monkeypatch, *responses, search_docs=None):
    """Point app.agent.runtime.get_graph() at a hermetic graph so answer()/
...
class TestAstreamEventsTurnUnattended:
    ...
    (pattern 8), used by app/turns/agent_worker.py's Redis Streams consumer,
    ...
    coverage for (tests/agent/test_durable_checkpoint.py)."""
