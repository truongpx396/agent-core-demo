"""Tests for cooperative mid-turn cancellation (app/agent.py's
...
tests/test_durable_checkpoint.py::TestAsyncSeeding; this file is the new
mechanism only.

Follows tests/test_streaming_terminal_events.py's established pattern:
...
class TestAstreamEventsTurnCancellation:
    ...
    def test_no_cancel_check_streams_normally_to_completion(self, monkeypatch):
        ...
        app/chat.py) passes no cancel_check at all — must behave exactly
