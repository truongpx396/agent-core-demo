"""Tests for app/telegram_channel.py — a fake async httpx client stands in
for a live Telegram API (see tests/test_model_resolver.py's _FakeResponse
for the sync-httpx equivalent), and app.telegram_channel.answer is
...
tests/test_durable_checkpoint.py) — async behavior here is driven the same
...
from app import telegram_channel
