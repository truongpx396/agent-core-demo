"""Tests for app/rate_limit.py's per-tenant HTTP rate limiting.
...
plugin here, see tests/test_queue.py's own module docstring) with a
hand-built Request, the same "test the function, not the framework
wiring" approach tests/test_api.py's module docstring already establishes
...
from app import metrics, rate_limit
...
class TestTenantRateLimitMiddleware:
    ...
    def test_storage_failure_fails_open_not_closed(self, monkeypatch):
        ...
        app/semantic_cache.py and app/moderation.py."""
        middleware = _fresh_middleware(monkeypatch, limit_per_minute=1)
