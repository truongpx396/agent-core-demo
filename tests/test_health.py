"""Tests for app/health.py's readiness checks — no live services: every
...
tests/test_sql_store.py/tests/test_tools.py already use.

No pytest-asyncio plugin is installed in this project (see
tests/test_queue.py's own module docstring) — async calls go through
`asyncio.run(...)` directly, same as every other async test here.
"""
import asyncio
import time

from app import health, qdrant_store, queue, sql_store
