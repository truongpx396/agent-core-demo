...
app/channels/chat.py and app/api/main.py never set `require_approval`, so a read-only
...
Uses app.agent.runtime's shared, DURABLE graph (app.agent.runtime.get_graph(), backed by
...
Run with: `python -m scripts.hitl_demo "what is 12 * 7?"`
"""
import getpass
import sys
import uuid
...
from app.agent.graph import resumability_error
from app.agent.runtime import get_graph
from app.core.config import DEFAULT_TENANT
from app.core.security import SecurityCtx

# Local dev ctx — see app/channels/chat.py's matching _LOCAL_CTX for why this
# process itself is the trusted boundary and no header-extraction gap
# applies here the way it does in app/api/main.py.
